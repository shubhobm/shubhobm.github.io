...
predict(m, newdata, interval="confidence", level = 0.90)
...
x <- runif(100)
## gives random points from uniform distribution
y <- 5 * x + rnorm(100)
## rnorm adds N(0,1) errors
m1 = lm(y~x)

par(mfrow=c(1,2))
plot(y ~ x, pch=19, cex=0.5, col=gray(.6),
     main='Confidence Interval Band')
